...
> Constraints (from PM):
> - No strategy changes without PM APPROVED.
> - Do NOT modify STRATEGY_KNOWLEDGE_BASE.md.
...
- @executor (implementation + evidence)
  1) Stage1 scaffolding compliance (per stage1-spec)
     - Ensure project tree matches exact layout:
       ...
       - logs/.gitkeep
     - Evidence-grade raw output in 02 report (single block):
       - preferred: `pwd && python3 --version && tree -a`
       - fallback: `find . -maxdepth 3 -type f | sort`
  ...
  2) config.py completeness vs White Paper (STRATEGY_KNOWLEDGE_BASE.md)
     ...
     - Must include at least:
       ...
       - WP §3.2: sweep buffer 0.15% (0.0015)
       - WP §3.3: confirmation candles = 3 consecutive candles above level
       - WP §4.1/§4.4: Gate 1 BTC risk-off: 1.0% drop over last 60 minutes
         - IMPORTANT: comparator ambiguity (“more than 1.0%” vs pseudocode `<= -0.010`) must be surfaced; do not silently choose.
  ...
  5) Workflow integrity / artifact retention (P0 blockers from ISSUES/DIALOGUE)
     - Canonical workflow artifacts must persist under `.opencode/workflow/*` after runs.
     - Fix/guard to prevent destructive selftest/pipeline from deleting or relocating canonical reports:
       - MUST NOT delete `.opencode/workflow/0{1..5}_*.md`
       - MUST NOT clobber `.opencode/workflow/ISSUES.md` or `.opencode/workflow/DIALOGUE.md`
     - Enforce `.opencode/workflow/ISSUES.md` append-only semantics (no truncation / regression).
     - Provide evidence (file+line references and brief explanation) in 02 report.
     - If destructive selftest cannot be run due to contention/timeouts, document exact blocker + recommended rerun procedure.
...
- @critic (canon/spec verification)
  ...
  3) Highlight and log mismatches/decisions needed:
     - Gate 1 comparator ambiguity at exactly -1.0%.
     - Stage1 logging limitation (blocked/suppressed events not emitted yet) if applicable.
...
- @auditor (risk + hygiene)
  ...
  2) Validate WP §7 logging posture (durable jsonl; plan for blocked-signal logging).
  3) Validate artifact retention policy and single source of truth:
     - `.opencode/workflow/*` is canonical; `_selftest_backup/*` is backup-only.
  4) Validate ISSUES.md integrity (append-only; no overwrite/regression).
  5) Validate secrets hygiene:
     - ensure logs never include TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID;
     - recommend/require a guard/test if missing.
  6) Write `.opencode/workflow/04_AUDITOR_REPORT.md` with findings + recommendations.

COLLECT:
- 02_EXECUTOR_REPORT.md: tree evidence, full config.py, raw command outputs (run proof), workflow retention/ISSUES append-only evidence, notes/risks.
- 03_CRITIC_REPORT.md: canon/spec mismatches, missing items, decisions needed, risks, verdict.
